\begin{frame}{Tâche : \only<1>{IR} \only<2>{mlsp \footfullcitenomarkleft{lagrange2019bandwidth}} \only<3>{synthèse}}
\begin{center}
\begin{tikzpicture}[mystyle]
\matrix [column sep=10mm,row sep=5mm,ampersand replacement=\&]
{
\node (i1) {\only<1-2>{X}\only<3>{\alert{x}}}; \&
\node [terminal] (i2) {p}; \&
\node  (i3) {\alert{\only<1>{y}\only<2>{Y}\only<3>{Y}}}; \\
};
...
\vspace{.8cm}
\only<1>{IR} \only<2>{séparation de source, inpainting, extension de bande, ...}
\end{center}
\visible<3>{
\begin{itemize}
...
\begin{itemize}
\item \structure{Jesse Engels}, magenta, Google brain (SFB)
